...
(function($, undefined) {
    ...
    $.extend(Colorpicker.prototype, {
        ...
        bindColor : function(target, settings) {
            ...
            var me = this;
            $(target).attr('readonly',this.settings.readonly).addClass('colorpicker-source')
                    .css('backgroundColor', $(target).val() || $(target).text())
                    .after('<a href="#"><img src="'
                            + this.settings.imageUrl
                            + '" class="colorpicker-icon" align="absmiddle"></a>')
        ...
        show : function(targetEl) {
            targetEl = $(targetEl);
            var eIC = targetEl.position();
            targetEl.css({
                        'z-index' : '9999',
                        'position' : 'relative'
                    });
            $('#colorPickerPanel').css({
                        'top' : eIC.top + targetEl.outerHeight() + 'px',
                        'left' : (eIC.left) + 'px',
                        'position' : 'absolute'
                    }).fadeIn('fast');
            ...
            var targetPre = targetEl.prev();
            var colorVal = targetPre.val();
            ...
            $('#colorPickerPanel a, #colorpickerPreview').unbind().mouseover(
                    function() {
                        ...
                        targetPre.val('#' + colorVal).css('background-color',
                                '#' + colorVal);
                    }).click(function() {
                        $(".colorPickerPanel a, #colorpickerPreview").unbind();
                        var colorVal = '#' + $(this).attr('rel');
                        targetPre.val(colorVal).css({
                                    'position' : 'static',
