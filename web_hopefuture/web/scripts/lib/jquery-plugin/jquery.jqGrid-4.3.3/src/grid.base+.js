...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			cmTemplate : {},
			idPrefix : "",
			emptyDataCaption : 'the data is empty',
			emptyDataCls : undefined,
			monitorResize : true,// monitor window resize event
			gridGetLength : false,// true get length
			loadedAdjustSize : true,//when grid loading, reset grid's size(width and height)
			resizable : false,//Defines if the column can be resized. Default it can't be resized
			title : true
		}, $.jgrid.defaults, pin || {});
		if(pin.rowList){
		...
		//Linder Wang start
		p.colNames.unshift('');
		p.colModel.unshift({width:3,colGetLength:false});
		if(p.resizable === false){
			jQuery.each(p.colModel, function(index,element){
				element.resizable = false;
			});
		}
		if(p.title === false){
			jQuery.each(p.colModel, function(index,element){
				element.title = element.title === true ? true : false;
			});
		}
		...
		addCell = function(rowId,cell,pos,irow, srvr) {
			...
			// get length
			var cm = ts.p.colModel[pos],width = cm.width,tooltip = '';
			if ((ts.p.gridGetLength === true && cm.colGetLength !== false ) || cm.colGetLength === true) {
				if ($.everbridge.platform.TextMetrics) {
				    //tooltip = 'class="b-tooltip" tooltip="true" tipcaption="' + v + '"';
				    var _tempTable = $('<div class="ui-jqgrid"><table class="ui-jqgrid-btable"><tr class="ui-widget-content jqgrow ui-row-ltr"><td></td></tr></table></div>');
					var size = $.everbridge.platform.TextMetrics.measure(
							_tempTable.find('tr td'), v), 
						textWidth = size.width;
					_tempTable.remove();
					if (textWidth > width) {
					    var actualLen = parseInt(width * v.length / textWidth) - 6;
						v = v.substring(0, actualLen) + '...';
					}
				}
			}
			
			return "<td role=\"gridcell\" "+prp + tooltip + ">"+v+"</td>" ;
		},
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			// custom update Linder Wang
			showEmptyDataCaption(gl == 0);
			return;
			// custom update Linder Wang end
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					...
					var lastpage = $.fmatter ? $.fmatter.util.NumberFormat(ts.p.lastpage,fmt):ts.p.lastpage;
					if(lastpage == 0 ){
						lastpage = 1;
					}
					$(sppg).html(lastpage);
		...
		// custom update Linder Wang
		showEmptyDataCaption = function(noData){
			if(noData){
				if($(ts.grid.bDiv).find('.ui-jqgrid-empty-data').length == 0){
			    	$('<div class="ui-jqgrid-empty-data">' + ts.p.emptyDataCaption + '</div>').appendTo(ts.grid.bDiv).addClass(ts.p.emptyDataCls || '');
				}
			}else{
				$(ts.grid.bDiv).find('.ui-jqgrid-empty-data').remove();
			}
		},
		
		adjustGridSize = function(){
			if(ts.p.loadedAdjustSize === true){
		        var jqgridP = $(ts).closest('div.jqgrid'),
		            widthP = jqgridP.width();
		        if(widthP > 0){
		        	$(ts).setGridWidth(widthP);
		        }
			}
		};
		// custom update Linder Wang end
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+" style='text-align:"+ this.p.colModel[i].align +";'>";
			idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
			thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+">"+jQuery.trim(ts.p.colNames[i]);
		...
		$("th",thr).each(function ( j ) {
			...
			}else{//add un sortable style Linder Wang
				$('>div',this).addClass('ui-jqgrid-unsortable');
			}
...
$.jgrid.extend({
	...
	resizeGrid : function(){
		var jqgridP = $(this).closest('div.jqgrid'),
		            widthP = jqgridP.width();
		        //console.info(widthP);
		        if(widthP > 0){
		        	$(this).setGridWidth(widthP);
		        }
	}
});
})(jQuery);
