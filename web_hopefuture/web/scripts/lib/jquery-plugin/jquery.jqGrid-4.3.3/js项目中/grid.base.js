...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addCell = function(rowId,cell,pos,irow, srvr) {
			...
			// get length
			var cm = ts.p.colModel[pos],width = cm.width;
		...
		showEmptyDataCaption = function(noData){
			if(noData){
				if($(ts.grid.bDiv).find('.g_warning').length == 0){
			    	var html = '<div class="g_warning">' + ts.p.emptyDataCaption + '</div>';
					$(ts.grid.bDiv).append(html);
				}
			}else{
				$(ts.grid.bDiv).find('.g_warning').remove();
			}
		},
		// custom update Linder Wang end
		...
		$("th",thr).each(function ( j ) {
			...
			if(sort) {
				...
				$(">div",this).addClass('no-underline');
			}
