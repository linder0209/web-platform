...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+ "style='text-align:" + (ts.p.colModel[i].align || "left") + "'>"+ts.p.colNames[i];
...
"use strict";	
	...
	$.fmatter.util = {
		...
		DateFormat : function (format, date, newformat, opts)  {
			...
			if( !isNaN( date - 0 ) && String(format) == "u") {
				//Unix timestamp
				timestamp = new Date( parseFloat(date)*1000 );
			} else if( !isNaN( date - 0 ) && String(format) == "U") {
				//timestamp 
